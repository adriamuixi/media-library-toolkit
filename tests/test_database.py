...
class DatabaseTests(unittest.TestCase):
    def test_initialize_creates_marked_test_catalog(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(status.schema_version, 18)
    ...
    def test_reinitialization_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(second.schema_version, 18)
    ...
    def test_initialize_migrates_existing_foundation_catalog(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertIn("media_file", objects)
            self.assertIn("file_location", objects)
            self.assertIn("scan_error", objects)
            self.assertIn("scan_checkpoint", objects)
            self.assertIn("metadata_extraction", objects)
            self.assertIn("media_metadata", objects)
            self.assertIn("date_resolution_attempt", objects)
            self.assertIn("media_date_resolution", objects)
            self.assertIn("media_relation", objects)
            self.assertIn("hash_attempt", objects)
            self.assertIn("media_hash", objects)
            self.assertIn("import_batch", objects)
            self.assertIn("file_observation", objects)
            self.assertIn("media_item", objects)
            self.assertIn("observation_location_history", objects)
            self.assertIn("organization_plan", objects)
            self.assertIn("media_with_provenance", objects)
            self.assertIn("duplicate_summary", objects)
